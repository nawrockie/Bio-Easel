...
# make sure we have RF annotation unless --all, in which case alignments need to be the same
if(defined $in_gc) { 
...
foreach my $transfer_gc_tag (@transfer_gc_tag_A) {
  ...
  if($transfer_gc_tag eq "RF") {
    ...
    if(! $found_it) {
      die "ERROR, trying to transfer $src_gc_tag_A[$gc_idx] annotation, but MSA read from $src_alifile does not have $src_gc_tag_A[$gc_idx] annotation\n";
    }
  }
